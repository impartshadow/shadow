...
## Type
Multi-stage gate — partially code-enforced via `core/loop_guard.py`,
partially harness-enforced.
...
- **Enforcement:** Harness-side (prompt compliance). Code enforcement would
  require intercepting Edit tool calls — possible future upgrade.
...
- **Enforcement:** Code-enforced in `core/loop_guard.py` (graduated levels).
  Session startup `git log` check catches cross-session loops.
...
- **Trigger:** the user asks the same question or makes the same request twice
- **Precondition:** Acknowledge the miss in one sentence, then fix
- **Enforcement:** Harness-side. The repetition detection in the system prompt
  fires automatically for cross-session patterns.
...
- **Enforcement:** Harness-side. Git log check at startup is code-enforced.

### 5. Dead-end protocol
- **Trigger:** Blocked on approach A
- **Precondition:** Try at least one alternative before reporting blocked
- **Enforcement:** Harness-side. Pre-denial gate (separate contract) catches
  premature "I can't" responses.
- **Pattern:** If authenticated access is blocked, check whether a browser
  session exists that can be borrowed (Playwright cookie reuse).
