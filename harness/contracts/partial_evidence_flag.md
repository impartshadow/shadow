...
## Revenue $ claim guard — upstream prompt fixes (2026-06-15)
Beyond research/analysis responses, this contract also blocks `$N/mo`-style revenue figures unless the response cites `state/revenue.json` or a Stripe tool call. The contract has been firing 10x/24h on autonomous Claude sessions launched by `scripts/heartbeat.py` and `scripts/daily_commander_brief.py` Opus-synthesis prompts — the prompt baked the revenue numbers into the system text but never instructed Claude to cite the source. **Upstream fix (gap-closer 2026-06-15):** both `_SYNTHESIS_PROMPT` constants now require `[source: state/revenue.json]` as a non-negotiable closing tag whenever the response includes a revenue figure. The deterministic fallback strings (used when the synthesis call fails) also append the citation. This converts the contract from a recurring post-block into an upstream prompt-time guarantee.